...
function convertImage(block: NotionImageBlock): string {
  ...
  const caption = block.image.caption.length > 0 ? richTextToPlainText(block.image.caption) : ''
 ...
 */
export function replaceImageUrls(mdx: string, replacements: Map<string, string>): string {
  let processed = mdx
